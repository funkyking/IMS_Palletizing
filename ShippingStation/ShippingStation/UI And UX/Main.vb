...
#Region "Pallet Inserting"
        ...
        Try
            ...
            If conn.State = ConnectionState.Open Then
                ...
                Sqlcmd.CommandText = "SELECT DISTINCT 
                                        CS.[Master Container ID],CS.[Master Container Name],
                                        CS.[Work Order], CS.[Sub Group], CS.[Pallet No], CS.[Shift],
                                        CM.[Completed]
    ...
    Private Function CheckOrderMasterStatusComplete()
        ...
        Try
            ...
            Dim PoNumberInput = ""
            If PoNumber.Contains("NULL") Then
                PoNumberInput = "wom.[Po Number] IS NULL"
            Else
                PoNumberInput = $"wom.[Po Number] = '" & PoNumber & "'"
            End If
            ...
            If conn.State = ConnectionState.Open Then
                ...
                SQLcmd.CommandText = $"SELECT DISTINCT
                                        ...
                                        WHERE wos.[Work Order] = '" & WorkOrder & "'
                                        AND " & PoNumberInput & "
                                        AND wos.[Sub Group] = '" & SubGroup & "'
    ...
    Private Function AddItemtoDbContainer(ByVal WID, ByVal WO, ByVal SG, ByVal PN, ByVal SFT) As Boolean
        ...
        Try
            If conn.State = ConnectionState.Open Then
                ...
                cmdCheck.Parameters.AddWithValue("@MCN", GlobalVariables.ContainerName)
                ...
                If rowCount = 0 Then
                    ...
                    Sqlcmd.CommandText = "INSERT INTO [CRICUT].[CUPID].[ContainerShipping]
                        ...
                        VALUES (NEWID(), '" & GlobalVariables.MasterContainerID.ToString() & "', 
                                            '" & GlobalVariables.ContainerName & "' , '" & WID & "', 
                                            '" & WO & "' ,'" & SG & "', 
                ...
                If NewEntry Then
                    Dim NewEntryQuery = "INSERT INTO [CRICUT].[CUPID].[ContainerMaster]
                                        ...
                                        WHERE NOT EXISTS (
                                            ...
                                            OR [Master Container Name] = '" & GlobalVariables.ContainerName & "'
                                        );"
                ...
                If CompleteStatus Then
                    Dim CompleteQuery As String = "UPDATE [CRICUT].[CUPID].[ContainerMaster]
                                        ...
                                        BEGIN
                                            ...
                                            ([Master Container ID], [Master Container Name], [Completed], [UserID], [Date], [IsDeleted])
                                            VALUES
                                            ('" & GlobalVariables.MasterContainerID.ToString() & "', '" & GlobalVariables.ContainerName & "', 1, '" & GlobalVariables.UserID & "', GETDATE(), 0);
                                        END"
                ...
                If Conn.State = ConnectionState.Open Then
                    ...
                    cmd.CommandText = "Insert INTO [CRICUT].[CUPID].[ContainerShipping]
                        ([Loading ID]
                              ...
                              ,[IsDeleted])
                        VALUES (NEWID(), '" & GlobalVariables.MasterContainerID.ToString() & "', 
                                            '" & GlobalVariables.ContainerName & "' , '" & item.WorkOrderID.ToString() & "', 
...
#Region "Generate Docs (pqf, excel, etc..)"
    ...
    Private Function CreatePdf()
        Try
            ...
            Dim font As XFont = New XFont("Bahnschrift Semilight Condensed", 12, XFontStyle.Regular)
            ...
            Dim containerNoText As String = $"[Container: {containerNo_txtbx.Text}]"
            ...
            Dim palletCountText As String = $"[Total Pallets: {totalPallets.ToString()}]"
        ...
        Try
            ...
            If conn.State = ConnectionState.Open Then
                ...
                sqlcmd.CommandText = "SELECT DISTINCT 
                                            ...
                                            ,cs.[Shift]
                                        FROM [CRICUT].[CUPID].[ContainerShipping] cs
                                        INNER JOIN [CRICUT].[CUPID].[WorkOrderMaster] wom ON cs.[Word Order ID] = wom.[Work Order ID]
                                        WHERE cs.[Master Container Name] = '" & GlobalVariables.ContainerName & "'
                ...
                While reader.Read()
                    ...
                    masterItem.WorkOrderID = reader("Word Order ID")
...
#Region "InputBox Controls"
    ...
    Private Sub addContainerNo_btn_Click(sender As Object, e As EventArgs) Handles addContainerNo_btn.Click
        Try
            ...
            If checkMasterContainerExistence(inputString) Then
                GlobalVariables.CID_Existance = True
            End If

            If Not inputString = "" AndAlso Not inputString = Nothing Then
                GlobalVariables.ContainerName = inputString

                If Not GlobalVariables.CID_Existance Then
                    GlobalVariables.MasterContainerID = System.Guid.NewGuid()
...
#End Region
    ...
    Private Sub execute_btn_Click(sender As Object, e As EventArgs) Handles execute_btn.Click
        Try
            ...
            ListBox1.Items.Add("Passed Items:")
            ...
            ListBox1.Items.Add("Failed Items:")
