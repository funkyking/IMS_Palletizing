...
Public Class frmProduction
	...
	Private Sub LoadComboBox()
		...
		If Conn.State = ConnectionState.Open Then
			...
			'                  FROM [CRICUT].[CUPID].[WorkOrderStatus] WHERE [PalletScanCompleted]='True'"
			SQLcmd.CommandText = "SELECT DISTINCT [Work Order]
	...
	Private Sub WorkOrderBox_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbWorkOrderBox.SelectedIndexChanged
		...
		Try
			...
			If Conn.State = ConnectionState.Open Then
				...
				Dim strsql = "Select [Sub Group] from [CRICUT].[CUPID].[WorkOrderMaster] where [Work Order]='" + cmbWorkOrderBox.Text + "'" 'And [Sub Group] is not null Order By [Sub Group] ASC"
				...
				If ds.HasRows Then
					While ds.Read
						...
						If Not ds.IsDBNull(ds.GetOrdinal("CountPalletizing")) Then
							totalordercount.Text = ds.Item("CountPalletizing")
						Else
							totalordercount.Text = "0"
						End If
...
skippallet:
	...
	Private Sub ScanBtn_Click(sender As Object, e As EventArgs) Handles ScanBtn.Click
		...
		'	MessageBox.Show("This Pallet Is already Palletized!", "Pallet Done Palletizing", MessageBoxButtons.OK, MessageBoxIcon.Error)
		'	Exit Sub
		'End If
...
here:
	...
	Private Function CheckSerialNumber()
		For i As Integer = 0 To DataGridView1.Rows.Count - 1
			...
			If Conn.State = ConnectionState.Open Then
				...
				Dim dsTest = SQLcmd.ExecuteReader
				If dsTest.HasRows Then
					dsTest.Read()
					DataGridView1.Rows(i).Cells("Serial No").Style.BackColor = Color.LimeGreen
					'statuslbl.Text = (" Serial No : " & (ds.Item("Serial No")))
					'statuslbl.Text = "Duplicate Serial No Found..." & vbCr & "PO No : " & ds.Item("Work Order") & vbCr & "MR No. : " & ds.Item("Sub Group") & vbCr & "Pallet No : " & ds.Item("Pallet No") & vbCr & "Carton No : " & ds.Item("Carton")
					'statuslbl.Text = "Duplicate Carton"
				Else
		...
		'Return res
	End Function
	Private Function CheckPalletScanComplete()
		For i As Integer = 0 To DataGridView1.Rows.Count - 1
			Dim res As Boolean
			Dim Conn = New SqlConnection(connstr)
			Conn.Open()
			If Conn.State = ConnectionState.Open Then
				...
				SQLcmd.CommandText = "SELECT [Pallet No],[Shift],[Work Order],[Sub Group],[PalletScanCompleted]
									  FROM [CRICUT].[CUPID].[WorkOrderStatus] 
									  WHERE [Work Order] = '" & cmbWorkOrderBox.SelectedItem & "' AND [Sub Group] = '" & cmbSubGroup.SelectedItem & "' AND [Pallet No] = '" & PalletBox.SelectedItem & "' AND [Shift] = '" & Shift.SelectedItem & "' AND [PalletScanCompleted] = 'True' "
				...
				Else
					...
					statuslbl.Text = ""
					statuslbl.Text = "Pallet Incomplete"
				End If
	...
	Private Function CheckSerialNumberScan()
		...
		If Conn.State = ConnectionState.Open Then
			...
			SQLcmd.CommandText = "SELECT wm.[Work Order],wm.[Sub Group],w.[Pallet No],wm.[Shift]
						
									  FROM [CRICUT].[CUPID].[WorkOrderStatus] wm
									  INNER JOIN [CRICUT].[CUPID].[WorkOrder] w
									  ON wm.[Work Order ID]=w.[Work Order ID]
									  WHERE w.[Serial No] = '" & txtMasterScan.Text & "'"
			...
			If ds.HasRows Then
				While ds.Read
					cmbWorkOrderBox.Text = ds.Item("Work Order")
					cmbSubGroup.Text = ds.Item("Sub Group")
					PalletBox.SelectedItem = ds.Item("Pallet No").ToString()
					Shift.Text = ds.Item("Shift")
					Dim testpallet As String
					testpallet = PalletBox.SelectedItem
				End While
			Else
				statuslbl.Text = ""
				statuslbl.Text = "Serial No Not Exist"
			End If
		End If
		Conn.Close()
	...
	Private Function UpdateCountSQL()
		...
		If Conn.State = ConnectionState.Open Then
			...
			SQLcmd.CommandText = "UPDATE  [CRICUT].[CUPID].[WorkOrderMaster] 
											 SET [CountPalletizing]='" & Integer.Parse(totalordercount.Text) & "'
	...
	Private Function UpdateCompleteSQL()
		...
		If Conn.State = ConnectionState.Open Then
			...
			'                         WHERE [Work Order ID] = '" & WID.ToString & "' "
			SQLcmd.CommandText = "UPDATE  [CRICUT].[CUPID].[WorkOrderStatus] 
											 SET [PalletizingCompleted] = '1'
									 WHERE [Work Order ID] = '" & WID.ToString().Trim() & "' OR [Work Order] = '" & cmbWorkOrderBox.SelectedItem.ToString().Trim() & "'"
	...
	Private Function UpdateCompletePalletizing()
		Try
			Dim Conn = New SqlConnection(connstr)
			Conn.Open()
			If Conn.State = ConnectionState.Open Then
				...
				SQLcmd.CommandText = "UPDATE  [CRICUT].[CUPID].[WorkOrderStatus] 
											 SET [PalletizingCompleted]= '1'
									 WHERE ([Work Order ID] = '" & WID.ToString.Trim() & "' OR [Work Order] = '" & cmbWorkOrderBox.SelectedItem.ToString().Trim() & "')
									 AND [Pallet No] = '" & PalletBox.SelectedItem.ToString().Trim() & "'"
	...
	Private Function LoadGrid()
		...
		'DataGridView1.Rows(1).Cells("Serial No").Style.BackColor = Color.Green
		'Dim test As String = DataGridView1.Rows(1).Cells("Serial No").Value
	...
	Private Sub DeleteToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DeleteToolStripMenuItem.Click
		...
		If res = DialogResult.Yes Then
			...
			If Conn.State = ConnectionState.Open Then
				Dim SQLcmd = New SqlCommand
				SQLcmd.Connection = Conn
				SQLcmd.CommandText = "DELETE FROM [CUPID].[WorkOrderPalletizing]
	...
	Private Function checkPalletScan(ByVal serialnumber As String)
		Try
			'[Cricut_MES].[dbo].[ProductUnit] database
			Dim Conn = New SqlConnection(connstr)
			Conn.Open() 'Open the Connection
			If Conn.State = ConnectionState.Open Then
				...
				SQLcmd.CommandText = "SELECT w.[Pallet No],w.[Serial No],wm.[Work Order],wm.[Sub Group]
									  ...
									  INNER JOIN [CRICUT].[CUPID].[WorkOrder] w
									  ON wm.[Work Order ID]=w.[Work Order ID]
									  WHERE w.[Serial No] = '" & serialnumber & "'AND [Pallet No] = '" & PalletBox.Text & "'"
				'WHERE [Work Order ID]='" & WID.ToString & "' AND [Serial No]='" & txtSearch.Text & "'"
	...
	Private Function FinishScan1()
		...
		If Timer1.Enabled Then
			...
			If statuslbl.Text <> "" Then
				Exit Function
			Else
				If lblOption.Text = "1" Or debug_chckbx.CheckState = CheckState.Checked Then
					If CheckDuplicateSerial(txtS1.Text) = False Then
						statuslbl.Text = "Duplicated Serial"
						Exit Function
					End If
	...
	Private Sub search()
		...
		If Conn.State = ConnectionState.Open Then
			...
			SQLcmd.Connection = Conn
			SQLcmd.CommandText = "SELECT [Pallet No]
							  FROM [CRICUT].[CUPID].[WorkOrderPalletizing] WHERE [Work Order ID]='" & WID.ToString & "' AND [Serial No]='" & txtSearch.Text & "'"
			Dim ds = SQLcmd.ExecuteReader
			If ds.HasRows Then
				While ds.Read
	...
	Private Sub txtMasterScan_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtMasterScan.KeyPress

		If e.KeyChar = ChrW(Keys.Enter) Then
			Dim checkMaster As Integer = txtMasterScan.Text.IndexOf(",")
	...
	Private Sub adminDebug_btn_Click(sender As Object, e As EventArgs) Handles adminDebug_btn.Click
		Try
	...
	Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
		Try
			Dim result As DialogResult = MessageBox.Show("This will Force this Pallet To Complete Palletizing" & vbCrLf &
															"Do you want to Proceed ?",
															"Force Complete Palletizing",
