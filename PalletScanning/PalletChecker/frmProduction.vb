...
Public Class frmProduction
	...
	Dim reprintExcelFilePath = ""


	Dim temporaryBypass = False
	...
	Private Function Initialize()
		...
		If GlobalVariables.AdminAccess Then
	...
	Private Sub MoCompleted_chkbx_CheckedChanged(sender As Object, e As EventArgs) Handles MoCompleted_chkbx.CheckedChanged
		Try
			LoadComboBox2()
	...
	'Improved Version by Paul (27/10/2023)
	Private Sub LoadComboBox2()
		...
		If Conn.State = ConnectionState.Open Then
			...
			If MoCompleted_chkbx.Checked = True Then
				query = "SELECT DISTINCT [Work Order]
						  FROM [CRICUT].[CUPID].[WorkOrderMaster] WHERE [Delete]='False'"
			Else
				query = "SELECT DISTINCT [Work Order]
							FROM [CRICUT].[CUPID].[WorkOrderMaster] 
							WHERE [Delete]='False'
							AND ([Count] < [Total Order Count])"
			End If
			...
			If ds.HasRows Then
				...
				While ds.Read
					workOrders.Add(ds.Item("Work Order").ToString())
				End While

				' Custom sorting using a lambda expression
				workOrders.Sort(Function(a, b)
									' sort by the length of the work order string
									Return a.Length.CompareTo(b.Length)
								End Function)

				For Each item In workOrders
					If Not cmbWorkOrderBox.Items.Contains(item) Then
						cmbWorkOrderBox.Items.Add(item)
					End If
				Next

				'cmbWorkOrderBox.Items.AddRange(workOrders.ToArray())
			End If
	...
	Private Sub WorkOrderBox_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbWorkOrderBox.SelectedIndexChanged
		...
		While ds.Read
			If Not cmbSubGroup.Items.Contains(ds.Item(0)) Then
		...
		'strsql = "Select [PO Number] from [CRICUT].[CUPID].[WorkOrderMaster] where [Work Order]='" + cmbWorkOrderBox.Text + "'" 'And [Sub Group] is not null Order By [Sub Group] ASC"
		'ds = cmd.ExecuteReader
		'If ds.HasRows Then
		'    While ds.Read
		'        txtPO.Text = ds.Item("PO Number")
		'    End While
	...
	Private Function GetPO()
		...
		If Conn.State = ConnectionState.Open Then
			...
			If ds.HasRows Then
				While ds.Read
					If Not ds.IsDBNull(ds.GetOrdinal("PO Number")) Then
						txtPO.Text = ds.Item("PO Number").ToString()
					Else
						txtPO.Text = "NULL"
					End If
				End While
		...
		If Conn.State = ConnectionState.Open Then
			...
			If ds.HasRows Then
				While ds.Read
					...
					If ds.IsDBNull(ds.GetOrdinal("Suffix")) Then
						Suffix = "NULL"
					Else
			...
			SQLcmd.CommandText = "SELECT DISTINCT CAST([Pallet No] AS INT) AS [Pallet No]
			...
			If ds.HasRows Then
				While ds.Read
					For Each items In PalletBox.Items
						If items = ds.Item("Pallet No").ToString() Then
							GoTo skippallet
						End If

					Next

					PalletBox.Items.Add(ds.Item("Pallet No").ToString())
...
here:
		...
		If Conn.State = ConnectionState.Open Then
			...
			SQLcmd.CommandText = "SELECT COUNT(w.[Serial No]) AS [cnt]
									FROM (
										...
										AND w.[Pallet No] = '" & query & "' AND wm.[Delete] = 'False'
									) AS w"
			Dim ds = SQLcmd.ExecuteReader
			If ds.HasRows Then
				While ds.Read
	...
	Private Sub DeleteToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DeleteToolStripMenuItem.Click
		...
		If Not DataGridView1.CurrentCell.ColumnIndex = 1 Then
			SerialNo = DataGridView1.Rows(DataGridView1.CurrentCell.RowIndex).Cells(1).Value()
		Else
			SerialNo = DataGridView1.CurrentCell.Value.ToString()
		End If

		Dim res = MessageBox.Show("Confirm Delete Item" & vbCrLf & "Serial No: " & DataGridView1.CurrentRow.Cells("Serial No").Value.ToString, "Confirm Operation", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Exclamation)
	...
	Public Function PrintReport()
		...
		Else
			...
			sourceWorkSheet.Range("B9").Value = r1.qty
			If r1.Description.Length > 2 And r1.Description.Length < 15 Then
				sourceWorkSheet.Range("B10").Value = $"{r1.Line} [{r1.Description}]"
			Else
	...
	Public Function PrintReportAndStoreExcel()
		...
		Dim ActualPalletNo = Integer.Parse(PalletBox.SelectedItem.ToString())

		'sourceWorkSheet.Name = ("Pallet " & i)
		'sourceWorkSheet.Range("B2").Value = r1.PalletNo
		sourceWorkSheet.Range("B2").Value = ActualPalletNo
		sourceWorkSheet.Range("B3").Value = r1.PartNo
		sourceWorkSheet.Range("B4").Value = r1.WorkOrder
		sourceWorkSheet.Range("B5").Value = r1.PONumber
		sourceWorkSheet.Range("B6").Value = r1.ProdDate.ToString("dd/MM/yyyy")
	...
	Private Sub SubGroup_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbSubGroup.SelectedIndexChanged
		LoadInfo()
		UpdateCartonCount(WID.ToString())
		CancelBtn_Click(Nothing, Nothing)
		EnsurePalletInWorkOrderStatus()
	...
	Private Function releaseComExcel()
		Try
			Dim objectsToRelease() As Object = {xlApp, sourceWorkBook, sourceWorkSheet}
			For Each obj As Object In objectsToRelease
	...
	Private Function reprint()
		...
		For i = 1 To PalletBox.Text
			...
			' Create a QR Code with QrCoder
			Dim qrStrdata = $"{r1.WorkOrder},{r1.PONumber},{r1.SubGroup},{i.ToString()},{r1.Shift}"
			Dim qrgen As New QRCoder.QRCodeGenerator()
			Dim qrdata = qrgen.CreateQrCode(qrStrdata, QRCodeGenerator.ECCLevel.Q)
			Dim qrcode As New QRCode(qrdata)
			Dim qrImage = qrcode.GetGraphic(2)
			...
			' Update Serial Carton Number Count to newest order
			UpdateCartonCount(r1.WorkOrder.Trim())
			...
			If Conn.State = ConnectionState.Open Then
				Dim SQLcmd = New SqlCommand
				SQLcmd.Connection = Conn
				SQLcmd.CommandText = "SELECT DISTINCT p.[Serial No],w.[Sub Group],p.[Work Order ID],p.[Index],p.[Carton],p.[Production Date]
										FROM [CRICUT].[CUPID].[WorkOrder] p   
										INNER JOIN [CRICUT].[CUPID].[WorkOrderMaster] w
										ON p.[Work Order ID]=w.[Work Order ID]   
										Where p.[Work Order ID]='" & WID.ToString & "' 
										AND [Sub Group]='" & cmbSubGroup.Text & "' 
										AND [Pallet No]='" & i & "' 
				...
				If ds.HasRows Then
					...
					While ds.Read
						...
						If cnt <= 40 Then
							...
							If Not ds.IsDBNull(ds.GetOrdinal("Serial No")) Then
								c1.Cells(i1, 2).Value = ds.Item("Carton")
							End If
				...
				If i > 1 Then
					...
					Dim filePath1 As String = (localdir & "\bin\Result\" & r1.WorkOrder & "_" & r1.SubGroup & ".xls")
					reprintExcelFilePath = filePath1
					Dim xlobj As New Excel.Application
					Dim xlobj1 As New Excel.Application
					Dim w As Workbook
					Dim w1 As Workbook
	...
	Private Function UpdateCartonCount(ByVal workID As String)
		Dim conn As New SqlConnection(connstr)
		Try
			conn.Open()
			If conn.State = ConnectionState.Open Then
	...
	Private Sub PrintReportSingularPallet()
		Try
			...
			' Insert The current Pallets Serial And Carton Values
			Dim conn As New SqlConnection(connstr)
			Try
				conn.Open()
				If conn.State = ConnectionState.Open Then
					Dim query = "SELECT [Serial No],[Carton]
								  FROM [CRICUT].[CUPID].[WorkOrder]
								  WHERE [Work Order ID] = '" & r1.WID.ToString() & "'
								  AND [Pallet No] = '" & palletCount.ToString() & "'
								  ORDER BY [Production Date] ASC"
					Using sqlcmd As New SqlCommand(query, conn)
						Dim ds = sqlcmd.ExecuteReader()
						If ds.HasRows Then
							While ds.Read
								currentSerial = ds.Item("Serial No").ToString()
								currentCarton = ds.Item("Carton").ToString()

								' Set the cell value in the current column and row
								sourceWorkSheet.Range($"{SerialColumn}{rowCount}").Value = currentSerial 'Row.Cells(1).Value.ToString()
								sourceWorkSheet.Range($"{CartonColumn}{rowCount}").Value = currentCarton 'Row.Cells(0).Value.ToString()
			...
			'	currentSerial = Row.Cells(1).Value.ToString()
			'	currentCarton = Row.Cells(0).Value.ToString("0000")
	...
	Sub ReorderWorksheetsNumerically(WorkbookPath As String)
		Try
			...
			Dim worksheetList As New List(Of Excel.Worksheet)

			' Iterate through worksheets and add them to the list
			For Each xlSheet As Excel.Worksheet In xlWorkbook.Sheets
				If xlSheet.Name.StartsWith("Pallet ") Then
					worksheetList.Add(xlSheet)
				End If
			Next

			' Sort the worksheets by the numerical part of their name
			worksheetList = worksheetList.OrderBy(Function(sheet)
													  Return Integer.Parse(sheet.Name.Replace("Pallet ", ""))
												  End Function).ToList()

			' Move the worksheets to the new order
			For i As Integer = 0 To worksheetList.Count - 1
				worksheetList(i).Move(Before:=xlWorkbook.Sheets(i + 1))
			Next
	...
	Private Sub kill_Excel()
		Try
			...
			Dim xlp2() As Process = Process.GetProcessesByName("Microsoft Excel")
	...
	Public Sub OpenReprintExcelCreation()
		Try
			...
			Try
				...
				Dim ows As Excel.Worksheet = CType(worksheet, Excel.Worksheet)
				ows.PageSetup.PaperSize = Excel.XlPaperSize.xlPaperA4
				ows.PrintOutEx(From:=1, To:=1) ' Print only the first page
			Catch ex As Exception
				Try
					' Handle any printing errors

					' Create the PDF file path using the same directory and Excel file name
					Dim pdfFilePath As String = System.IO.Path.ChangeExtension(WorkBookPath, "pdf")
	...
	Private Sub ContextMenuStrip1_Opening(sender As Object, e As CancelEventArgs) Handles ContextMenuStrip1.Opening
		' Check if a cell is selected and visible
	...
	Private Sub DataGridView1_MouseClick(sender As Object, e As MouseEventArgs) Handles DataGridView1.MouseClick
		If e.Button = MouseButtons.Right Or e.Button = MouseButtons.Left Then
			Dim hitTest = DataGridView1.HitTest(e.X, e.Y)
	...
	Private Sub debug_btn_Click(sender As Object, e As EventArgs) Handles debug_btn.Click
		If GlobalVariables.AdminAccess Then
	...
	' Trigger display for EnsurePalletInWorkOrderStatus()
	Private Function showLabels_EPWS(ByVal show As Boolean)
		Try
			If show = True Then
				cmbWorkOrderBox.Enabled = False
				PalletBox.Enabled = False
	...
	Private Function EnsurePalletInWorkOrderStatus()
		Dim conn As New SqlConnection(connstr)
		Try
			conn.Open()
			If conn.State = ConnectionState.Open Then
				...
				If PalletBox.Items.Count > 0 Then
					...
					For Each item In PalletBox.Items
						...
						Dim WOS_Query = "SELECT [Work Order ID]
									  ...
									  WHERE ([Work Order ID] = '" & r1.WID.ToString().Trim() & "' or [Work Order] = '" & r1.WorkOrder.Trim() & "')
									  AND [Sub Group] = '" & r1.SubGroup.Trim() & "' 
