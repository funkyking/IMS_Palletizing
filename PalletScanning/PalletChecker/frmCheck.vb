...
skippallet:
    ...
    Private Sub GetSerialCount()
        'Updated 30/10/2023 by Paul
        'The maximum count is retrieving from datagrid not database which will cause issue when user reopens application
        ...
        Try
            conn.Open()
            If conn.State = ConnectionState.Open Then
                Dim query = "SELECT [Work Order ID]
                              ,[Po Number]
                              ,[Work Order]
                              ,[Sub Group]
                              ,[Quantity]
                ...
                Using cmd As New SqlCommand(query, conn)
                    ...
                    If ds.HasRows Then
                        While ds.Read
                            If Not ds.IsDBNull(ds.GetOrdinal("Quantity")) Then
                                serialCount = ds.Item("Quantity")
                                maxSerial = ds.Item("Quantity")
                            End If
                        End While
                        ds.Close()
                    End If
